...
- 示例时间采用 UTC+03:00：首次出现为 `2026-09-06 15:52:30 (UTC+03:00)`，最近出现为 `2026-09-09 13:01:27 (UTC+03:00)`，与 event_alert 卡片的时间字段同为单行展示（Figma 稿曾把偏移单独换行，实机验证后发现在飞书客户端里过于占高度，已改回单行）；原始 UTC 时间保留在源数据中。飞书卡片没有"按查看者本地时区显示"的概念，只能固定选一个时区渲染成文本；这个时区是每条"项目 → 群"映射各自配置的（/admin/sentry-projects 管理页面，IANA 时区名，如 `Asia/Shanghai`），不是网关全局一个——不同地区的群可以各配各的（比如中国群 `Asia/Shanghai`，利雅得群 `Asia/Riyadh`）。没配就用默认值 `Asia/Riyadh`（即 UTC+3，跟示例一致）。
