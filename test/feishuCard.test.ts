...
describe('buildFeishuCard', () => {
  it('renders environment · type-text in the header, and project/summary/fields/trailing/button in the body', () => {
    const card = buildFeishuCard({
      ...
      trailingField: { labelKey: 'locationHint', value: 'Screen' },
    ...
    expect((card.header as any).title.i18n.en_us).toBe('development · Error event');
    expect((card.header as any).title.i18n.zh_cn).toBe('development · 错误事件');
    ...
    // body order: project (no label) -> summary (no label) -> fields grid -> hr -> trailing -> button
    ...
    expect(zh[3]).toEqual({ tag: 'hr' });
    expect(zh[4]).toEqual({ tag: 'div', text: { tag: 'plain_text', content: '定位线索\nScreen' } });
    expect(zh[5]).toEqual({
      tag: 'action',
      actions: [
        { tag: 'button', text: { tag: 'plain_text', content: '查看错误事件' }, type: 'primary', url: 'https://sentry.example.com/issues/1/' },
      ],
    });
    expect(en[2].fields[0].text.content).toBe('Level\nerror');
    expect(en[4].text.content).toBe('Location hint\nScreen');
    expect(en[5].actions[0].text.content).toBe('View error event');
  ...
  it('truncates a very long summary in the body instead of stuffing it into the (single-line) header', () => {
    ...
    expect((card.header as any).title.i18n.en_us).toBe('Error event');
  ...
  it('missing summary falls back to a translated "(untitled)" placeholder, not an empty line', () => {
    const card = buildFeishuCard({ titleKey: 'error', color: 'red', fields: [] });
  ...
  it('project display prefers name, then slug, then "#id"; omitted entirely when all are unknown', () => {
    const withName = buildFeishuCard({ titleKey: 'error', color: 'red', fields: [], projectId: '4', projectSlug: 'p', projectName: 'P' });
    expect((withName.i18n_elements as any).zh_cn[0].text.content).toBe('P');

    const withSlug = buildFeishuCard({ titleKey: 'error', color: 'red', fields: [], projectId: '4', projectSlug: 'p' });
    expect((withSlug.i18n_elements as any).zh_cn[0].text.content).toBe('p');

    const withIdOnly = buildFeishuCard({ titleKey: 'error', color: 'red', fields: [], projectId: '4' });
    expect((withIdOnly.i18n_elements as any).zh_cn[0].text.content).toBe('#4');

    const withoutProject = buildFeishuCard({ titleKey: 'error', color: 'red', fields: [] });
  ...
  it('no environment: header title is just the type text, no leading separator', () => {
    const card = buildFeishuCard({ titleKey: 'issueCreated', color: 'red', fields: [] });
  ...
  it('action/status field values are translated when recognized; unrecognized ones pass through raw', () => {
    const known = buildFeishuCard({ titleKey: 'issueCreated', color: 'red', fields: [{ labelKey: 'action', value: 'created' }] });
    ...
    const unknown = buildFeishuCard({ titleKey: 'notification', color: 'grey', fields: [{ labelKey: 'action', value: 'some_weird_action' }] });
    expect((unknown.i18n_elements as any).zh_cn.at(-1).fields[0].text.content).toBe('动作\nsome_weird_action');
  });

  it('standaloneFieldsBefore render as their own lines ahead of the fields grid', () => {
    const card = buildFeishuCard({
      titleKey: 'ruleAlert',
      color: 'red',
      summary: 'x',
      standaloneFieldsBefore: [{ labelKey: 'triggeredRule', value: 'Production error alert' }],
      fields: [{ labelKey: 'level', value: 'error' }],
    });
    const zh = (card.i18n_elements as any).zh_cn;
    // project omitted (no project data) -> summary -> triggeredRule (standalone) -> fields grid
    expect(zh[1]).toEqual({ tag: 'div', text: { tag: 'plain_text', content: '触发规则\nProduction error alert' } });
    expect(zh[2].fields).toBeDefined();
  });

  it('no trailing field -> no hr; no url -> no button (fallback/notification shape)', () => {
    const card = buildFeishuCard({
  ...
  it('button label follows the alert category (issue/metric share one label across their action variants)', () => {
    const issueResolved = buildFeishuCard({ titleKey: 'issueResolved', color: 'green', fields: [], url: 'https://x/1' });
    const issueArchived = buildFeishuCard({ titleKey: 'issueArchived', color: 'grey', fields: [], url: 'https://x/1' });
    ...
    const metricWarning = buildFeishuCard({ titleKey: 'metricWarning', color: 'orange', fields: [], url: 'https://x/1' });
    expect((metricWarning.i18n_elements as any).zh_cn.at(-1).actions[0].text.content).toBe('查看指标告警');

    const notification = buildFeishuCard({ titleKey: 'notification', color: 'grey', fields: [], url: 'https://x/1' });
